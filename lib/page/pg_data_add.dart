...
class PageDataAdd extends StatefulWidget {
  const PageDataAdd({Key? key}) : super(key: key);
  @override
  _PageDataAddState createState() => _PageDataAddState();
}

class _PageDataAddState extends State<PageDataAdd> {
  XFile? _image;
  String? urlupload,val_nama,val_kategori,val_harga;
  String? val_user;
            ...
            children: [
                      ...
                      Container(
                        decoration: BoxDecoration(
                            image: DecorationImage(image:
                                    (_image==null)
                                        ?AssetImage(Gambar("default.jpg")) as ImageProvider
                    ...
                    StreamBuilder<QuerySnapshot>(
                      ...
                      builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
                        if(!snapshot.hasData){
                          return AniLoad();} else
                        {
                          data_kategori= snapshot.data;
                          list_kategori.clear();
                          snapshot.data!.docs.forEach((element) {
                            list_kategori.add(element['Nama'].toString());
                          });
                          return DropdownStyle(
                            mylist: list_kategori,
                            title: "Kategori",
                            cari: false,
                            valueText: val_kategori,
                            onSaved: (val){
                              DataForm("Kategori", val);
                            },
                            onChange: (val){
                              val_kategori=val;
                            },
              ...
              Padding(
                ...
                child: ElevatedButton(
                    onPressed: (){
                      //tulis fungsi klik disini
                      (_image!=null)? uploadgambar():toast("Harus Ada Gambar!",color: Warna.red);
                    },
                    style: ElevatedButton.styleFrom(
      ...
      FirebaseFirestore.instance
        ...
        uploadTask.whenComplete(() async {
          ...
          await FirebaseFirestore.instance.runTransaction((
            ...
            await myTransaction.set(FirebaseFirestore.instance.collection("data").doc(newid),dataForm);
          });
          toast("Berhasil Diupload",color: Warna.green);
          Navigator.pop(context);
          Navigator.pop(context);
        }).catchError((onError) {
          print(onError);
          Navigator.pop(context);
          toast("Upload Gagal,Cek kembali isian",color: Warna.red);
        });
